...
export interface Edit { oldText: string; newText: string }
export interface AppliedEditsResult { baseContent: string; newContent: string }
...
export function applyEditsToNormalizedContent(
  ...
  for (let i = 0; i < normalizedEdits.length; i++) {
    if (normalizedEdits[i].oldText.length === 0) {
      const msg = normalizedEdits.length === 1
  ...
  const usedFuzzyMatch = normalizedEdits.some(
  ...
  for (let i = 0; i < normalizedEdits.length; i++) {
    ...
    const fuzzyContent = normalizeForFuzzyMatch(replacementBaseContent);
